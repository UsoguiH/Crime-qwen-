...
backend/   FastAPI + SQLAlchemy 2 (async) + SQLite(WAL)|Postgres — single asyncio worker owns the pipeline
           ...
           bbox_2d 0–1000, decoupled per-object grounding + zoom refine) → s4 aggregate entities →
           s5 timeline (pure code) → s7 narratives (citation-validated) → s8 annotate (badges, face blur) →
           s9 render (WeasyPrint PDF/A + docxtpl DOCX + court ZIP bundle)
```

Per-photo quick analysis (detect+grounding only) and grounded photo Q&A ship alongside the full pipeline. One OpenAI-SDK client covers all model modes (OpenRouter · DashScope · vLLM · mock). Model calls are logged (tokens/latency/cost/served-backend), retried with exponential backoff, JSON-schema-validated, and repaired on failure. Prompts live in `backend/app/prompts/` — versioned, their SHA-256 recorded per analysis run and printed in the report appendix.
...
docker run --rm -v athar_athar-data:/data verapdf/cli -f 3b /data/reports/<run-id>/report.pdf
```

Seed a demo case: `docker compose exec backend python scripts/seed_demo.py`
Ground-truth evaluation harness (COCO): `backend/eval/` (`REPORT.md`, `LOG.md`).
...
| `backend/app/pipeline/` | pipeline stages + decoupled grounding (`grounding.py`) |
| `backend/app/modelclient/` | unified Qwen3-VL client (api/local/mock) + JSON repair + budget |
| `backend/eval/` | ground-truth evaluation harness + measured reports |
